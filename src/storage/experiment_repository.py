...
class ExperimentRepository(BaseRepository):
    def create_mru_experiment(self, exp_data: ExperimentCreate, physics_data: dict):
        try:
            ...
            res_exp = (
                self.client.table("experimentos")
                .insert({"nombre": exp_data.nombre, "tipo": exp_data.tipo})
                .execute()
            )
            ...
            res_physics = (
                self.client.table("ensayos_mru").insert(physics_data).execute()
            )
    ...
    def create_mrua_experiment(self, exp_data: ExperimentCreate, physics_data: dict):
        try:
            ...
            physics_data["experimento_id"] = nuevo_id
            res_physics = (
                self.client.table("ensayos_mrua").insert(physics_data).execute()
            )
    ...
    def get_all(self) -> list:
        try:
            ...
            # CORRECCIÓN AL ERROR 500: Poblar el campo 'detalle' para satisfacer el response_model de FastAPI
            for exp in listado:
                exp_id = exp["id"]
                if exp["tipo"] == "MRU":
                    det = (
                        self.client.table("ensayos_mru")
                        .select("*")
                        .eq("experimento_id", exp_id)
                        .execute()
                    )
                else:
                    det = (
                        self.client.table("ensayos_mrua")
                        .select("*")
                        .eq("experimento_id", exp_id)
                        .execute()
                    )
            ...
            self._handle_error("get_all", str(e))

    def get_by_id(self, exp_id: int) -> dict | None:
        response = (
            self.client.table("experimentos").select("*").eq("id", exp_id).execute()
        )
