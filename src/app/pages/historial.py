...
st.set_page_config(
    page_title="PhysiLab - Historial de Ensayos", page_icon="🗄️", layout="wide"
)
...
for exp in experimentos:
    ...
    with st.container(border=True):
                    ...
                    if nuevo_nombre != nombre_actual:
                        try:
                            put_res = requests.put(
                                f"{API_URL}/{exp_id}?nuevo_nombre={nuevo_nombre}"
                            )
                            if put_res.status_code == 200:
                                st.toast(
                                    f"¡Experimento {exp_id} actualizado!", icon="✅"
                                )
                                st.rerun()
        ...
        if st.session_state.analizar_id == exp_id:
            ...
            if not detalle:
                st.warning(
                    "No se encontraron variables numéricas para este ensayo en las tablas de física."
                )
            else:
                try:
                    ...
                    posicion_inicial = float(
                        detalle.get(
                            "posicion_inicial", detalle.get("posicion_origen", 0.0)
                        )
                    )
                    tiempo_total = float(
                        detalle.get("tiempo", detalle.get("tiempo_total", 5.0))
                    )
                    velocidad_inicial = float(
                        detalle.get("velocidad_inicial", detalle.get("velocidad", 0.0))
                    )
                    aceleracion = (
                        float(detalle.get("aceleracion", 0.0))
                        if tipo == "MRUA"
                        else 0.0
                    )
                    ...
                    for t in pasos_tiempo:
                        ...
                        datos_grafico.append(
                            {"Tiempo (s)": t, "Posición (m)": x, "Velocidad (m/s)": v}
                        )
                    ...
                    fig_analisis.update_traces(
                        line_color="#4f6dd7" if tipo == "MRUA" else "#7ec4cf"
                    )
                    ...
                    with met_col2:
                        st.metric(
                            "Velocidad Inicial ($v_0$)", f"{velocidad_inicial} m/s"
                        )
                    with met_col3:
                        if tipo == "MRUA":
                    ...
                    st.error(
                        f"Error al procesar las variables físicas del diccionario: {eval_err}"
                    )
