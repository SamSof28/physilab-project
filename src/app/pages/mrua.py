...
st.info("Define la incógnita física y completa los datos conocidos para resolver el ensayo.")
...
with st.form("form_mrua"):
    ...
    with c2:
        xf = st.number_input("Posición Final (m) [Dejar 0 si falta]", min_value=0.0, value=0.0)
...
if enviar:
    ...
    try:
        res = requests.post(f"{BACKEND_URL}/calculate/mrua", params={"nombre": nombre}, json=payload, timeout=20)
        ...
        if res.status_code == 201:
            ...
            posiciones = [det["posicion_inicial"] + (det["velocidad_inicial"] * t) + (0.5 * det["aceleracion"] * (t**2)) for t in pasos_tiempo]
            
            fig = px.area(
                x=pasos_tiempo, y=posiciones,
