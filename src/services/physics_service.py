...
class PhysicsService:
    ...
    def resolver_y_guardar_mru(self, nombre: str, datos: MRUSchema):
        ...
        if datos.distancia is None:
            ...
            datos.tiempo = self._division_segura(
                datos.distancia, datos.velocidad, "tiempo"
            )
        elif datos.velocidad is None:
            datos.velocidad = self._division_segura(
                datos.distancia, datos.tiempo, "velocidad"
            )
    ...
    def resolver_y_guardar_mrua(self, nombre: str, m: MRUASchema):
        ...
        if m.posicion_final is None and m.tiempo is not None:
            m.posicion_final = (
                m.posicion_inicial
                + (m.velocidad_inicial * m.tiempo)
                + (0.5 * m.aceleracion * m.tiempo**2)
            )

        if (
            m.velocidad_final is None
            and m.aceleracion is not None
            and m.tiempo is not None
        ):
            m.velocidad_final = m.velocidad_inicial + (m.aceleracion * m.tiempo)

        if m.tiempo is None:
            m.tiempo = self._resolver_tiempo_cuadratico(
                m.aceleracion, m.velocidad_inicial, m.posicion_inicial, m.posicion_final
            )
